...
def get_token():
  ...
  #private_key_data = os.environ["SK"].replace("\\n", "\n").encode('utf-8')
    ...
    response = requests.post(url, data=data, headers={'Content-Type': 'application/x-www-form-urlencoded'}, verify=tlsrootcert)
...
def bulk_download(to_dir=consts.PREPARED_DIR):
   ...
   # print(response.json())
    ...
    try:
        response = requests.get("https://api."+os.environ["HMZ_SERVER"]+"/v1/vaults/"+os.environ["VAULTID"]+"/operations/prepared",headers={ 'Authorization' : 'Bearer '+ token },stream=True,verify=tlsrootcert) 
        ...
        with open(file_path, 'r') as f_j:
            ...
            for i in jcontents["accounts"]:
                content = { "vaultId" : jcontents["vaultId"] , "accounts" : [ i ] , "transactions" : [  ] , "manifests": []}
                filepath = os.path.join(consts.PREPARED_DIR,i["accountId"])
                with open(filepath, 'w') as outfile:
                    json.dump(content,outfile)
            for i in jcontents["manifests"]:
                content = { "vaultId" : jcontents["vaultId"] , "accounts" : [  ] , "transactions" : [  ] , "manifests": [ i ]}
...
def bulk_upload(from_dir=consts.SIGNED_DIR):
    ...
    content = { "accounts" : v_ac , "transactions" :  v_tx  , "manifests": v_ma , "vaults": v_va }
    ...
    try:
        response = requests.post("https://api."+os.environ["HMZ_SERVER"]+"/v1/vaults/operations/signed",headers={ 'Authorization' : 'Bearer '+ token },files=data, verify=tlsrootcert) 
        response.raise_for_status()
    except RequestException as e:
        print(f"An error occurred: {e}")
        return "ERROR",500
